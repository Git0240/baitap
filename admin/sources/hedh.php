...
function get_item() {
    ...
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=hdh&act=man");

    $sql = "select * from #_hedh where id_hdh='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=hdh&act=man");
    $item = $d->fetch_array();
}
...
function delete_item() {
    ...
    if (isset($_GET['id'])) {
        ...
        // xoa item
        $sql = "delete from #_hedh where id_hdh='" . $id . "'";
        if ($d->query($sql))
            header("Location:index.php?com=hdh&act=man");
        else
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=hdh&act=man");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=hdh&act=man");
}
?>
