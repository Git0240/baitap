...
function get_item() {
    ...
    if (!$id)
        transfer("Không nhận được dữ liệu", "index.php?com=lk&act=man");

    $sql = "select * from #_linhkien where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
        transfer("Dữ liệu không có thực", "index.php?com=lk&act=man");
    $item = $d->fetch_array();
}
...
function save_item() {
    ...
    if ($id) {
        ...
        if ($d->update($data))
            ...
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=lk&act=man");
    }else {
        ...
        if ($d->insert($data))
            ...
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=lk&act=man");
    }
}
...
function delete_item() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=lk&act=man");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=lk&act=man");
}
?>
