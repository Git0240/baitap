...
switch ($act) {
    case "man":
        get_items();
        $template = "congtrinhtieubieu/items";
        break;
    case "add":
        get_hang();
        get_kg();
        get_hdh();
        get_nlk();
        $template = "congtrinhtieubieu/item_add";
        break;
...
function get_items() {
    ...
    $sql = "select * from #_congtrinhtieubieu order by id desc";
    $d->query($sql);
...
function get_item() {
    ...
    $sql = "select * from #_congtrinhtieubieu where id='" . $id . "'";
    $d->query($sql);
    if ($d->num_rows() == 0)
...
function save_item() {
    ...
    if ($id) {
        ...
        $d->setWhere('id', $id);
        if ($d->update($data))
            redirect("index.php?com=congtrinhtieubieu&act=man");
        else
            transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=congtrinhtieubieu&act=man");
    }else {
        $file_name = fns_Rand_digit(0, 9, 8);
        ...
        if ($d->insert($data))
            ...
            transfer("Lưu dữ liệu bị lỗi", "index.php?com=congtrinhtieubieu&act=man");
    }
}
...
function delete_item() {
    ...
    if (isset($_GET['id'])) {
        ...
        if ($d->num_rows() > 0) {
            ...
            $sql = "delete from #_congtrinhtieubieu where id='" . $id . "'";
            $d->query($sql);
        }
        ...
        if ($d->query($sql))
            ...
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=congtrinhtieubieu&act=man");
    } else
        transfer("Không nhận được dữ liệu", "index.php?com=congtrinhtieubieu&act=man");
}
?>
