...
function ChuoiNgauNhien($sokytu){ // tao randomkey chuoi ngau nhien
        $chuoi="ABCDEFGHIJKLMNOPQRSTUVWXYZWabcdefghijklmnopqrstuvwxyzw123456789";
        for ($i=0; $i < $sokytu; $i++){
...
{	
	...
	{
		return mysqli_real_escape_string($str, $id_connect);
	}
...
*/
	global $d;
	$time = 600; // 10 giay tuc la 600/60giay=10 giay
	$ssid = session_id();
...
function check_yahoo($nick_yahoo='nthaih'){
	$file = @fopen("http://opi.khachhang.com/online?u=".$nick_yahoo."&m=t&t=1","r");
	$read = @fread($file,200);
